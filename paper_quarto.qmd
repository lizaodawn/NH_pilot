...
@tbl-coltable_absolute_freqencies, @tbl-coltable_Gsigned and @tbl-coltable_PMI display the top 20 contextual collocates associated with the word "india", respectively sorted by PMI, absolute frequency signed $G^2$ scores. 
...
#| fig-cap: Scatter plot of signed $G^2$ scores and absolute frequency in textual collocation analysis
...
While in the top 20 list sorted with PMI scores, as shown in @tbl-colloc_frequency_compare_PMI, all of the results shows higher frequency than that in the reference context.
```{r}
...
The PMI word list shows more descriptive information about the theme related to India in *Natural History*.
