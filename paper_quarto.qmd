...
And the full text of the entire work, segregrating in paragraphs, are scraped and stored in TXT format as a **reference corpus**. 
...
The **target corpus** consists of all text related to India in paragraphs, containing `r prettyNum(n_tokens(flist_target_all))` tokens and `r n_types(flist_target_all)` types, with the type-token ratio of `r round(n_types(flist_target_all)/n_tokens(flist_target_all), 2)`. 
