...
The **target corpus** consists of all text related to Indian subcontinent (with coordinates in range of *8°4'N-37°6'N and 68°7'E to 97°25'E*), segregated by paragraphs, containing `r prettyNum(n_tokens(tar_flist))` tokens and `r n_types(tar_flist)` types, with the type-token ratio of `r round(n_types(tar_flist)/n_tokens(tar_flist), 2)`.
