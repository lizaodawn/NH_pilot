...
Textual collocation refers to the frequently occurring word with the target word in the context. Different from the surface collocation, which only taking the word left and right to the target word, the word span in this case refers to the "textual units" around the target word. Setting "India" as the target word in the corpus of all paragraphs in *Natural History*, the `mclm::text_cooc()` function returns the frequency of words observed in the context of "India". As mentioned in the Methodology Section (@sec-method), the returned scores were filtered for those having PMI scores higher than 2 and signed $G^2$ scores higher than 3.84, in order to gain a strong and significant word list co-occurring with word "India" in the context. 
